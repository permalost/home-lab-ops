...
model(s) — `main` (the primary Hermes model) and the sibling `aux/`
directory (auxiliary roles: title/compression/approval/web_extraction).
...
model ever comes back, only `aux/endpointslice.yaml` needs to change.

To point the cluster at a different model or backend: update
`endpointslice.yaml`'s address/port in `main/` and/or `aux/`, update this
